#!/bin/bash
# Script de compilação do paper DeepBridge Fairness
# Garante que as referências bibliográficas sejam processadas corretamente
...
if [ -f "main.pdf" ]; then
    ...
    REF_COUNT=$(pdftotext main.pdf - | grep "^\[" | wc -l)
    echo "📚 Referências bibliográficas: $REF_COUNT"

    if [ "$PAGES" -eq 17 ] && [ "$REF_COUNT" -eq 24 ]; then
        echo "✅ Paper completo com todas as referências!"
    else
        echo "⚠️  Verificar: esperado 17 páginas e 24 referências"
    fi
else
